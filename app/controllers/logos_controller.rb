class LogosController < ApplicationController
  ...
  def update
    respond_to do |format|
      if not @logo.image.nil? && @logo.favicon.nil?
        logo_params[:image]= @logo.image
        logo_params[:favicon]= @logo.image
      end
  ...
  private
    ...
    def logo_params
      params.require(:logo).permit(:image, :name, :favi, :facebook, :flickr,:instagram ,:estaEdicaoTitle , :estaEdicaoDesc , :contatoEmail, :contatoEnd)
    end
end
